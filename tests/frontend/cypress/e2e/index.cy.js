...
//Handle the "process is not defined" error from Firebase
...
    cy.get('nav ul a').contains('Home');
    cy.get('nav ul a').contains('About');
    cy.get('nav ul a').contains('Features');
    cy.get('nav ul a').contains('How it Works');
    cy.get('nav ul a').contains('Contact');
  });
// Navbar Link Functionality: Integration Test
  it('navbar links scroll to correct sections', () => {
    cy.get('nav ul a[href="#about"]').click();
    cy.get('#about').should('be.visible');
    cy.get('nav ul a[href="#features"]').click();
...
  // Get Started Button: Integration Test
